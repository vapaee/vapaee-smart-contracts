...
namespace vapaee {
    ...
    struct slug {
        ...
        constexpr explicit slug(): value() {
            init(0);
        }

        constexpr explicit slug(const unsigned long &i): value() {
            init(i);
        }

        constexpr explicit slug(const int &i): value() {
            init((const unsigned long)i);
        }

        constexpr explicit slug(const char* s): value() {
            init(0);
            init(s);
        }

        constexpr explicit slug(std::string_view s): value() {
            init(0);
            init(s);
        }
