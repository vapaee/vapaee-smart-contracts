

// char_to_value
#define ERROR_CTV_1 "ERR-CTV-1: character is not in allowed character set for names: "


// check_name_from_string
#define ERROR_CNFS_1 "ERR-CNFS-01: contract name is too long to be a valid name: "
#define ERROR_CNFS_2 "ERR-CNFS-02: contract name can not be empty: "
#define ERROR_CNFS_3 "ERR-CNFS-03: character not valid for a name: "
#define ERROR_CNFS_4 "ERR-CNFS-04: thirteenth character of contract name cannot be a letter that comes after j: "

// check_symbol_code_from_string
#define ERROR_CSCFS_1 "ERR-CSCFS-01: string is too long to be a valid symbol_code: "
// TODO: ERROR text missing
#define ERROR_CSCFS_2 "ERR-CSCFS-02: "

// check_asset_from_string
#define ERROR_ACFS_1 "ERR-CAFS-01: the string has not a valid asset format and can not be parsed as such: "
